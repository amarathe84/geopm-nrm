...
CC = /usr/tce/packages/openmpi/openmpi-4.0.0-gcc-4.9.3/bin/mpicc
#/opt/openmpi/4.0/gnu/bin/mpicc
...
### platform requires.
OTHER_CFLAGS = -D_GEOPM_
OTHER_LIB = -L$(GEO_INSTALL_PATH)/lib -L$(LIBMSR_INSTALL_PATH)/lib \
			-Xlinker -rpath $(GEO_INSTALL_PATH)/lib \
			-lgeopm -fopenmp
OTHER_INCLUDE = -I$(GEO_INSTALL_PATH)/include -I$(LIBMSR_INSTALL_PATH)/include
...
${CoMD_EXE}: ${BIN_DIR} CoMD_info.h ${OBJECTS} 
	...
	${CC} ${OTHER_INCLUDE} ${CFLAGS_no_omp} ${OTHER_CFLAGS} -c CoMD.c -fopenmp
	${CC} ${OTHER_INCLUDE} ${CFLAGS_no_omp} ${OTHER_CFLAGS} -c timestep.c -fopenmp
	${CC} ${CFLAGS_no_omp} ${OBJECTS} ${LDFLAGS} ${OTHER_LIB} -o comd.geo
