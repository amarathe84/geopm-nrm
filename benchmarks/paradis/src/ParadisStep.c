...
extern uint64_t nodeforce_rid, rebalance_rid, migrate_rid, recyclegn_rid, sortnative_rid, commsend_rid, cellcharge_rid, stepint_rid, resetglide_rid, getdensity_rid, deltaplasticstrain_rid, inittopology_rid, splitmultinodes_rid, crossslip_rid, handlecollisions_rid, commsendremesh_rid, fixremesh_rid, applydeltastress_rid, rebalance_rid, sortnative_rid; 
/*
 ...
 *          any forces.
 */
            int err = geopm_prof_enter(nodeforce_rid);
            NodeForce(home, FULL);
            err = geopm_prof_exit(nodeforce_rid);
            err = geopm_prof_enter(rebalance_rid);
            Rebalance(home, DLB_USE_FORCECALC_COUNT);
            err = geopm_prof_exit(rebalance_rid);
 ...
 */
            err = geopm_prof_enter(migrate_rid);
            Migrate(home);
            err = geopm_prof_exit(migrate_rid);
            err = geopm_prof_enter(recyclegn_rid);
            RecycleGhostNodes(home);
            err = geopm_prof_exit(recyclegn_rid);
            err = geopm_prof_enter(sortnative_rid);
            SortNativeNodes(home);
            err = geopm_prof_exit(sortnative_rid);
            err = geopm_prof_enter(commsend_rid);
            CommSendGhosts(home);
            err = geopm_prof_exit(commsend_rid);
 ...
 */
        int err = geopm_prof_enter(cellcharge_rid);
        CellCharge(home);
        err = geopm_prof_exit(cellcharge_rid);
 ...
 */
        err = geopm_prof_enter(stepint_rid);
 ...
 */
        ...
        err = geopm_prof_exit(stepint_rid);
 ...
 */
        err = geopm_prof_enter(resetglide_rid);
        ResetGlidePlanes(home);
        err = geopm_prof_exit(resetglide_rid);
 ...
 */
        err = geopm_prof_enter(getdensity_rid);
        GetDensityDelta(home);
        err = geopm_prof_exit(getdensity_rid);

/*
 *      Calculate the new plastic strain.
 */
        err = geopm_prof_enter(deltaplasticstrain_rid);
        DeltaPlasticStrain(home);
        err = geopm_prof_exit(deltaplasticstrain_rid);
 ...
 */
        err = geopm_prof_enter(inittopology_rid);
        InitTopologyExemptions(home);
        err = geopm_prof_exit(inittopology_rid);
 ...
 */
        ...
        err = geopm_prof_enter(splitmultinodes_rid);
        SplitMultiNodes(home);
        err = geopm_prof_exit(splitmultinodes_rid);
 ...
 */
        err = geopm_prof_enter(crossslip_rid);
        CrossSlip(home);
        err = geopm_prof_exit(crossslip_rid);

/*
 *      Search for dislocation segments in close proximity to each other
 *      and if necessary handle any collision between them.
 */
        err = geopm_prof_enter(handlecollisions_rid);
        HandleCollisions(home);
        err = geopm_prof_exit(handlecollisions_rid);
        ...
        err = geopm_prof_enter(commsendremesh_rid);
        CommSendRemesh(home);
        err = geopm_prof_exit(commsendremesh_rid);
        TimerStop(home, COL_SEND_REMESH);

        TimerStart(home, COL_FIX_REMESH);
        err = geopm_prof_enter(fixremesh_rid);
        FixRemesh(home);
        err = geopm_prof_exit(fixremesh_rid);
 ...
 *      cycle.
 */
        err = geopm_prof_enter(applydeltastress_rid);
        ApplyDeltaStress(home, deltaStress);
        err = geopm_prof_exit(applydeltastress_rid);
