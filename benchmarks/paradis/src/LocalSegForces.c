...
#pragma omp parallel
        {
            ...
            VECTOR_ZERO(f1ThreadTot);
            VECTOR_ZERO(f2ThreadTot);

            GetThreadIterationIndices(numSegs, &threadID, &threadIterStart,
                                      &threadIterEnd);
...
/*
 *              Get the positions of the endpoints of the surface intersecting
 *              segment.  <p1S> is the position of the surface node.
 */
...
/*
 *              Surface normal is next 3 components of the buffer, but we
 *              don't need that here, so skip it.
 */
                bufIndex += 3;

/*
 *              Pull the burgers vector and sign from the buffer
 */
                burgSign = segList[bufIndex++];

                bVirtual[0] = -burgSign * segList[bufIndex++];
                bVirtual[1] = -burgSign * segList[bufIndex++];
                bVirtual[2] = -burgSign * segList[bufIndex++];
 ...
 */
                ...
                SemiInfiniteSegSegForce(p1S[0], p1S[1], p1S[2],
 ...
 *              has calcualted for the segment.
 */
                VECTOR_ADD(f1ThreadTot, f1);
                VECTOR_ADD(f2ThreadTot, f2);
            }
...
#pragma omp critical (VIRTUAL_SEG_FORCE)
            {
                VECTOR_ADD(f1, f1ThreadTot);
                VECTOR_ADD(f2, f2ThreadTot);
            }
        }

        return;
}
#endif  /* end ifdef _FEM */
