...
void Initialize(Home_t *home,int argc, char *argv[]) 
{
        ...
        uint64_t initialize_rid;
        int err = geopm_prof_region("paradisinit", GEOPM_REGION_HINT_COMPUTE, &initialize_rid);
        err = geopm_prof_enter(initialize_rid);
/*
 ...
 */
        ...
        err = geopm_prof_exit(initialize_rid);
        return;
}
